...
void koopa_update(struct Koopa* koopa, int xscroll) {
	...
	unsigned short tileright=tile_lookup((koopa->x >> 8)+16,(koopa->y>>8)+16,xscroll,0,map,map_width,map_height);
	unsigned short tileleft=tile_lookup((koopa->x >> 8),(koopa->y>>8)+16,xscroll,0,map,map_width,map_height);
	...
	if((tileleft >=1 && tileleft <= 6) || (tileleft >= 12 && tileleft <=17)){
		koopa->yvel=0;
		koopa->x+=1*256;
	}
	if((tileright >=1 && tileright <= 6) || (tileright >= 12 && tileright <=17)){
		koopa->yvel=0;
		koopa->x-=1*256;
	}
